%% Task 3 -- Newton minimization method with constant step gamma
...
while norm(gradfValue1) >= epsilon    % apply the Newton algorithm
    inPoint1 = inPoint1 - gammaConst * transpose(( double(inv( h(inPoint1(1), inPoint1(2))) ) * transpose(gradfValue1) ));
...
while norm(gradfValue2) >= epsilon    % apply the Newton algorithm
    inPoint2 = inPoint2 - gammaConst * transpose(( double(inv( h(inPoint2(1), inPoint2(2))) ) * transpose(gradfValue2) ));
...
while norm(gradfValue3) >= epsilon
    inPoint3 = inPoint3 - gammaConst * transpose(( double(inv( h(inPoint3(1), inPoint3(2))) ) * transpose(gradfValue3) ));
...
%% Starting Point (0,0)
figure('Name', ['Newton Method with constant gamma = ' num2str(gammaConst) '. Starting point =(' num2str(inPoint1(1)) ',' num2str(inPoint1(2)) ')']) 
fsurf(f) 
hold on
...
figure('Name', ['Newton Method with constant gamma = ' num2str(gammaConst) '. Starting point =(' num2str(initialPoint1(1)) ',' num2str(initialPoint1(2)) ')'])
fcontour(f)
hold on
scatter(Points1Const(:, 1), Points1Const(:, 2),'+','r')
hold on 
scatter(Points1Const(end,1),Points1Const(end, 2),'p','k')
...
figure('Name', ['f Values for each iteration. Newton Method with constant gamma = ' num2str(gammaConst) '. Starting point =(' num2str(inPoint1(1)) ',' num2str(inPoint1(2)) ')'])
...
%% Starting Point (-1,-1)
figure('Name', ['Newton Method with constant gamma = ' num2str(gammaConst) '. Starting point =(' num2str(inPoint2(1)) ',' num2str(inPoint2(2)) ')']) 
fsurf(f) 
hold on
...
ylabel('y','FontWeight','bold')
zlabel('f(x, y)','FontWeight','bold')
title(['Newton Method for constant gamma: ' num2str(gammaConst)], 'Starting Point (-1,-1)')
...
%% Starting Point (1,1)
figure('Name', ['Newton Method with constant gamma = ' num2str(gammaConst) '. Starting point =(' num2str(inPoint3(1)) ',' num2str(inPoint3(2)) ')']) 
fsurf(f) 
hold on
...
%%
IterationsPoint1Task3ConstGamma = size(Points1Const,1) ;
IterationsPoint2Task3ConstGamma = size(Points2Const,1) ;
IterationsPoint3Task3ConstGamma = size(Points3Const,1) ;
