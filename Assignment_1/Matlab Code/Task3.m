...
for iL = 1 : lNum
    [kf1, ~] = fibonacciMethod(indexF,lowLimit,UpperLimit,lMatf1(iL));   % for every lamda get k 
...
for iL = 1 : lNum
    [k, ~] = fibonacciMethod(indexF,lowLimit,UpperLimit,lMatf2(iL));   % for every lamda get k 
    kIterationsf2l(iL) = k;
end
...
for iL = 1 : lNum
    [k, ~] = fibonacciMethod(indexF,lowLimit,UpperLimit,lMatf3(iL));   % for every lamda get k 
    kIterationsf3l(iL) = k;
end
...
indexF = 1 ;
eConst = 0.001 ;
lValues = [ 0.005 0.008 0.02 0.05 0.08 0.1]  ;
...
for iV = 1 : length(lValues)
    [~ , limits] = fibonacciMethod(indexF,lowLimit,UpperLimit,lValues(iV)) ;
