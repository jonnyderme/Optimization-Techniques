...
for iL = 1 : lNum
    [kf1, ~] = derivativeMethod(indexF,lowLimit,UpperLimit,lMatf1(iL));   % for every lamda get k 
...
for iL = 1 : lNum
    [k, ~] = derivativeMethod(indexF,lowLimit,UpperLimit,lMatf2(iL));   % for every lamda get k 
    kIterationsf2l(iL) = k;
end
...
for iL = 1 : lNum
    [k, ~] = derivativeMethod(indexF,lowLimit,UpperLimit,lMatf3(iL));   % for every lamda get k 
    kIterationsf3l(iL) = k;
end
...
for iV = 1 : length(lValues)
    [~ , limits] = derivativeMethod(indexF,lowLimit,UpperLimit,lValues(iV)) ;
